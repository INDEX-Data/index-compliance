...
export function ComplianceDonut({ summary }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#e4e7ec] p-5">
      <p className="text-xs font-semibold text-[#505967] uppercase tracking-widest mb-4">Control Breakdown</p>
      <div className="flex items-center gap-5">
        ...
        <div className="space-y-2 flex-1">
          {[
            ...
            { key: 'Not Assessed', color: '#cad0d9', val: notAssessed  },
          ].map(({ key, color, val }) => (
            <div key={key} className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full shrink-0" style={{ background: color }} />
              <span className="text-[11px] text-[#505967] flex-1">{key}</span>
              <span className="text-[11px] font-semibold text-[#1c1d1f]">{val}</span>
            </div>
          ))}
        </div>
      </div>
