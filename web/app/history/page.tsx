  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-[22px] font-bold text-[#18181B] tracking-tight">Report History</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-32 text-center">
          <div className="w-14 h-14 rounded-2xl bg-white border border-[#E9E5DD] flex items-center justify-center mb-5 shadow-card">
            <FileText className="w-7 h-7 text-[#D4CFC5]" />
          </div>
          <h3 className="text-base font-semibold text-[#18181B] mb-2">No reports yet</h3>
        ...
        <div className="space-y-6">
          {frameworkIds.map(fwId => {
            ...
            return (
              <div key={fwId} className="bg-white rounded-xl border border-[#E9E5DD] overflow-hidden shadow-card">
                {/* Framework group header */}
                <div className="px-5 py-3 bg-[#FAFAF8] border-b border-[#F0EDE6] flex items-center gap-3">
                  <div className="w-7 h-7 rounded-lg bg-white border border-[#E9E5DD] flex items-center justify-center shrink-0">
                    <Shield className="w-3.5 h-3.5 text-[#9CA3AF]" />
                  </div>
                  <div className="flex-1">
                    <span className="text-[13px] font-bold text-[#18181B]">{rows[0].frameworkName}</span>
                  </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#F0EDE6]">
                      <th className="text-left px-5 py-3 text-[10px] font-semibold text-[#9CA3AF] uppercase tracking-widest">Date</th>
                      <th className="text-left px-5 py-3 text-[10px] font-semibold text-[#9CA3AF] uppercase tracking-widest">Score</th>
                      <th className="text-left px-5 py-3 text-[10px] font-semibold text-[#9CA3AF] uppercase tracking-widest">Δ vs prev</th>
                      <th className="text-left px-5 py-3 text-[10px] font-semibold text-[#9CA3AF] uppercase tracking-widest">Risk</th>
                      <th className="text-left px-5 py-3 text-[10px] font-semibold text-[#9CA3AF] uppercase tracking-widest">Controls</th>
                      <th className="text-right px-5 py-3 text-[10px] font-semibold text-[#9CA3AF] uppercase tracking-widest" />
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#F0EDE6]">
                    {rows.map((r, idx) => {
                      ...
                      return (
                        <tr
                          key={r.reportId}
                          className="hover:bg-[#FAFAF8] transition cursor-pointer"
                        ...
                        >
                          ...
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <div className="text-[12px] text-[#374151] flex items-center gap-1.5">
                                  <Calendar className="w-3 h-3 text-[#C4BFB5]" />
                                  {new Date(r.generatedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                                </div>
                                <div className="text-[10px] text-[#C4BFB5] mt-0.5 ml-4.5">
                                  {new Date(r.generatedAt).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}
                                </div>
                                <div className="text-[9px] text-[#D4CFC5] font-mono mt-0.5">{r.reportId}</div>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-2">
                              <div className="w-16 bg-[#F0EDE6] rounded-full h-1.5">
                          ...
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-1.5 text-[11px]">
                              ...
                              <span className="text-[#D4CFC5]">/ {r.summary.totalControls}</span>
                            </div>
                          </td>
                          ...
                          <td className="px-5 py-3.5 text-right" onClick={e => e.stopPropagation()}>
                            <div className="flex items-center justify-end gap-1">
                              <Link
                                ...
                                className="p-1.5 rounded-lg text-[#C4BFB5] hover:text-[#18181B] hover:bg-[#F7F5F1] transition"
                                title="View report"
                              >
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg text-[#C4BFB5] hover:text-[#B91C1C] hover:bg-[#FEF2F2] transition disabled:opacity-40"
                                title="Delete report"
                              >
                                {deleting === r.reportId
...
function DeltaCell({ delta }: { delta: number | null }) {
  if (delta === null) {
    return <span className="text-[11px] text-[#D4CFC5]">—</span>
  }
