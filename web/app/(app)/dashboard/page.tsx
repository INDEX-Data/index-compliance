...
function StatCard({ label, value, icon: Icon, iconColor, iconBg, sub }: StatCardProps) {
  return (
    <div className="bg-white rounded-xl border border-[#E9E5DD] p-5 shadow-card">
      ...
      <div className="text-[28px] font-bold text-[#18181B] tabular-nums leading-none tracking-tight">
        {value}
      </div>
      <div className="text-[12px] font-medium text-[#9CA3AF] mt-2">{label}</div>
      {sub && <div className="text-[11px] text-[#C4BFB5] mt-0.5">{sub}</div>}
    </div>
  )
}
...
function FrameworkCard({ report, history }: FrameworkCardProps) {
  ...
  const controls = [
    ...
    { label: 'Not Assessed', value: report.summary.notAssessed,  icon: HelpCircle,   color: '#9CA3AF', bg: '#F3F4F6' },
  ]
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E9E5DD] overflow-hidden shadow-card hover:shadow-card-hover transition-shadow">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#F0EDE6]">
        <div>
          <h2 className="text-[15px] font-semibold text-[#18181B] tracking-tight">{report.frameworkName}</h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1 text-[12px] font-semibold text-[#374151] hover:text-[#18181B] transition"
          >
        ...
        <div className="px-6 pb-5 border-t border-[#F0EDE6] pt-4">
          ...
          <ul className="space-y-1.5">
            {report.summary.topFindings.slice(0, 3).map((f, i) => (
              <li key={i} className="text-[12px] text-[#6B7280] flex items-start gap-2 leading-relaxed">
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-[13px] text-[#9CA3AF] mt-1">Compliance posture across all clients</p>
        </div>
        <Link
          href="/assess"
          className="inline-flex items-center gap-2 bg-[#18181B] hover:bg-[#27272A] text-white text-[13px] font-semibold px-4 py-2.5 rounded-lg transition-colors"
        >
      ...
      {latestReports.length === 0 ? (
        ...
        <>
          ...
          {/* ── Framework cards ── */}
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[13px] font-semibold text-[#374151]">Framework Posture</h2>
            <Link
              href="/history"
              className="text-[12px] text-[#9CA3AF] hover:text-[#374151] font-medium transition flex items-center gap-1"
            >
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      <div className="w-14 h-14 rounded-2xl bg-white border border-[#E9E5DD] flex items-center justify-center mb-5 shadow-card">
        <Building2 className="w-6 h-6 text-[#D4CFC5]" />
      </div>
      <h3 className="text-[15px] font-semibold text-[#18181B] mb-2">Add your first client</h3>
      <p className="text-[13px] text-[#6B7280] max-w-sm mb-8 leading-relaxed">
            ...
            <div key={s.step} className="flex flex-col items-center gap-1">
              <div className={`flex items-center gap-2 px-3.5 py-2 rounded-lg font-semibold border ${
                s.active
                  ...
                  : 'bg-white text-[#9CA3AF] border-[#E9E5DD]'
              }`}>
                <span className={`w-4 h-4 rounded-full text-[10px] font-bold flex items-center justify-center shrink-0 ${
                  s.active ? 'bg-[#C4A96D] text-[#18181B]' : 'bg-[#E9E5DD] text-[#9CA3AF]'
                }`}>{s.step}</span>
                {s.label}
              </div>
              {s.time && <span className="text-[10px] text-[#9CA3AF]">{s.time}</span>}
            </div>
            {i < arr.length - 1 && (
              <ArrowRight key={`arrow-${i}`} className="w-4 h-4 text-[#D4CFC5] mb-4 shrink-0" />
            )}
          </>
        ))}
      </div>
      ...
      <Link
        href="/clients"
        className="inline-flex items-center gap-2 bg-[#18181B] hover:bg-[#27272A] text-white text-[13px] font-semibold px-5 py-2.5 rounded-lg transition-colors"
      >
...
function EmptyNoAssessments({ firstClientName }: { firstClientName: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      <div className="w-14 h-14 rounded-2xl bg-white border border-[#E9E5DD] flex items-center justify-center mb-5 shadow-card">
        <Shield className="w-6 h-6 text-[#D4CFC5]" />
      </div>
      <h3 className="text-[15px] font-semibold text-[#18181B] mb-2">Run your first assessment</h3>
      <p className="text-[13px] text-[#6B7280] max-w-sm mb-8 leading-relaxed">
        {firstClientName
      ...
      <div className="flex items-center gap-3 mb-8 text-[13px] flex-wrap justify-center">
        ...
        <ArrowRight className="w-4 h-4 text-[#D4CFC5] mb-4 shrink-0" />
        <div className="flex flex-col items-center gap-1">
          <div className="flex items-center gap-2 px-3.5 py-2 rounded-lg font-semibold border bg-[#18181B] text-white border-transparent">
        ...
        <ArrowRight className="w-4 h-4 text-[#D4CFC5] mb-4 shrink-0" />
        <div className="flex flex-col items-center gap-1">
          <div className="flex items-center gap-2 px-3.5 py-2 rounded-lg font-medium border bg-white text-[#9CA3AF] border-[#E9E5DD]">
