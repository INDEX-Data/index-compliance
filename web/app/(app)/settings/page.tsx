...
function Section({
  ...
  return (
    <div className={`py-7 ${!last ? 'border-b border-border-subtle' : ''}`}>
      <div className="mb-5">
        <h3 className="text-[14px] font-semibold text-ink">{title}</h3>
        {description && (
          <p className="text-[13px] text-faint mt-1 leading-relaxed">{description}</p>
        )}
      </div>
      {children}
    </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-faint mt-0.5">Microsoft Entra ID</p>
              </div>
            </div>
    ...
    <div>
      ...
      >
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button
            ...
            className="px-4 py-2.5 text-[13px] font-semibold text-white bg-ink rounded-lg hover:bg-ink active:bg-[#0c0a09] disabled:opacity-50 disabled:cursor-not-allowed transition-colors whitespace-nowrap"
          >
      ...
      {(loading || pendingInvites.length > 0) && (
        <Section
          ...
          description="Awaiting acceptance. Copy the link to resend."
        >
          {loading ? (
            <div className="text-[13px] text-faint">Loading…</div>
          ) : pendingInvites.length === 0 ? (
            <p className="text-[13px] text-faint">No pending invites.</p>
          ) : (
      ...
      <Section
        title="Team Members"
        description="People with access to your clients and assessments."
        last
      >
        {loading ? (
          <div className="text-[13px] text-faint">Loading…</div>
          ...
          <div className="flex flex-col items-center py-8 text-center">
            ...
            <p className="text-[13px] font-medium text-faint">No team members yet</p>
            <p className="text-[12px] text-faint mt-1">
              Generate an invite link above to get started.
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {members.map((m) => (
              <div
                key={m.id}
                className="flex items-center gap-3 px-4 py-3 bg-[#F0FDF4] border border-[#BBF7D0] rounded-lg"
              >
                ...
                <span className="flex-1 text-[12px] font-mono text-ink truncate">{m.memberId}</span>
  ...
  return (
    <div>
      <Section
        ...
        description="Manage your third-party integration connections."
      >
        <button
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <p className="text-[13px] font-semibold text-ink">Manage Integrations</p>
              <p className="text-[11px] text-faint mt-0.5">
                ServiceNow, Splunk, Jira, Slack and more
              </p>
            </div>
          </div>
          <ExternalLink className="w-4 h-4 text-faint group-hover:text-ink transition" />
        </button>
      </Section>

      <Section
        title="Available Integrations"
        description="Tools you can connect to automate workflows."
        last
      >
          ...
          {tools.map((t) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-faint mt-0.5 truncate">{t.desc}</p>
              </div>
            </div>
          ))}
        </div>
        <button
          onClick={() => router.push('/integrations')}
          className="mt-3 text-[12px] text-faint hover:text-ink font-medium transition flex items-center gap-1"
        >
  ...
  return (
    <div>
      <Section
        ...
        description="Configure when Atlas sends you compliance alerts."
        last
      >
        <div className="flex flex-col items-center py-12 text-center">
          ...
          <p className="text-[13px] text-faint max-w-xs leading-relaxed">
...
const API_ROWS: {
  key: keyof Pick<
    PermissionsData,
    'graph' | 'graphBetaSecurity' | 'defender' | 'managementActivity'
  >
  label: string
  description: string
  permissions: string
}[] = [
  {
    ...
    permissions:
      'User.Read.All, Policy.Read.All, AuditLog.Read.All, DeviceManagementConfiguration.Read.All, SecurityEvents.Read.All',
  },
  {
  ...
  },
  {
    ...
    description:
      'Device risk scores, vulnerability management, CVEs, security recommendations, KQL hunting',
    permissions:
      'Machine.Read.All, Vulnerability.Read.All, AdvancedQuery.Read.All, Alert.Read.All (api.securitycenter.microsoft.com)',
  },
  {
      ...
      >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {status !== 'ok' && status !== 'unchecked' && (
                      <div className="mt-2 p-2.5 bg-white rounded-lg border border-border">
                        ...
                        {detail && <p className="text-[11px] text-[#D97706] mt-1.5">{detail}</p>}
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="mt-6 p-4 bg-[#EFF6FF] border border-[#DBEAFE] rounded-xl">
          ...
          <ol className="text-[12px] text-[#3B82F6] leading-relaxed space-y-1 list-decimal list-inside">
            <li>
              Go to{' '}
              <span className="font-medium">Azure Portal &gt; App registrations &gt; your app</span>
            </li>
            <li>
              Click <span className="font-medium">API permissions &gt; Add a permission</span>
            </li>
            <li>Select the API (Microsoft Graph, Defender, or Office 365 Management)</li>
            <li>
              Choose <span className="font-medium">Application permissions</span> and add the
              required ones
            </li>
            <li>
              Click <span className="font-medium">Grant admin consent</span>
            </li>
          </ol>
          <a
        ...
        <div className="max-w-3xl px-8 pb-16">
          ...
          <div className="pt-8 pb-2">
            ...
            <p className="text-[14px] text-faint mt-1.5">
              ...
              {activeTab === 'team' &&
                'Invite colleagues to share access and collaborate on assessments.'}
              {activeTab === 'integrations' &&
                'Connect Atlas to your existing security and productivity tools.'}
              {activeTab === 'api-access' &&
                'Check and manage Microsoft API permissions for deeper compliance data.'}
          ...
          {/* Tab content */}
          {activeTab === 'general' && (
            <GeneralTab tenantName={tenantName} clientCount={clientCount} loading={loadingStatus} />
          )}
