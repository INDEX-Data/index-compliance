...
export default function AssessPage() {
  ...
  const grouped = useMemo(() => {
    ...
    if (uncategorized.length > 0)
      map.set('general_security' as FrameworkCategory, [
    ...
    <div className="max-w-6xl px-8 lg:px-12 py-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-10">
        <div className="space-y-3">
          <h1 className="text-2xl font-bold tracking-tight text-ink">Run Assessment</h1>
          ...
          {Array.from(grouped.entries()).map(([cat, fws]) => (
            <section key={cat}>
              ...
              <div className="flex items-center gap-4 mb-6">
                <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-faint whitespace-nowrap">
                ...
                {fws.map((fw) => {
                  ...
                  return (
                    ...
                    >
                      {/* Logo */}
                      <div className="mb-6">
                        <div className="w-12 h-12 rounded-xl bg-canvas border border-border flex items-center justify-center overflow-hidden p-1.5">
                          {logo ? (
                            <Image
                              src={logo}
                              alt={fw.name}
                            ...
                            <Shield className="w-6 h-6 text-faint" strokeWidth={1.5} />
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex-1">
                        <h3 className="text-lg font-bold tracking-tight text-ink mb-2">
                          {fw.name}
                        </h3>
                        {fw.description && (
                          <p className="text-[13px] text-muted leading-relaxed mb-4 opacity-80">
                            {fw.description}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-auto pt-2">
                        ...
                        {fw.implemented ? (
                          isLaunching ? (
                            ...
                            <ArrowRight
                              className="w-5 h-5 text-ink opacity-0 group-hover:opacity-100 transition-opacity"
                              strokeWidth={1.5}
                            />
                          )
                        ) : (
                          <span className="text-[10px] font-medium bg-canvas text-faint px-2.5 py-1 rounded-full flex items-center gap-1">
      ...
      {!loading && clients.length > 0 && (
        <div className="mt-28">
          <div className="flex items-center gap-3 mb-10">
            <span className="text-[11px] font-extrabold uppercase tracking-[0.3em] text-faint">
            ...
            <div className="space-y-4">
              ...
              {clients.length === 1 ? (
                <div className="flex items-center gap-4 py-4 px-6 bg-white border border-slate-100 rounded-xl">
                  ...
                  <div>
                    <span className="text-base font-semibold text-ink">{selectedClient?.name}</span>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4 py-4 px-6 bg-white border border-slate-100 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-faint">Typically 30–120 seconds per framework</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* ── Footer ── */}
      <footer className="mt-32 pt-8 flex justify-between items-center text-[10px] font-bold uppercase tracking-[0.15em] text-faint">
              ...
              >
                ...
                <div className="px-6 pt-6 pb-4 border-b border-border">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-canvas border border-border flex items-center justify-center overflow-hidden p-1.5">
                        {logo ? (
                          <Image
                            src={logo}
                            alt={confirmFw.name}
                          ...
                          <Shield className="w-5 h-5 text-faint" strokeWidth={1.5} />
                        )}
                      </div>
                      <div>
                        <h2 className="text-lg font-bold text-ink">Confirm Assessment</h2>
                        <p className="text-xs text-faint mt-0.5">{confirmFw.name}</p>
                      </div>
                    </div>
                    <button
                      onClick={() => setConfirmFw(null)}
                      className="p-1 rounded-lg hover:bg-canvas transition-colors text-faint hover:text-muted"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                ...
                <div className="px-6 py-5 space-y-4">
                  ...
                  <div className="bg-canvas rounded-xl p-4">
                    <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-faint mb-3">
                      Target Tenant
                    </p>
                    <div className="flex items-center gap-3">
                  ...
                  <div className="flex gap-3">
                    <div className="flex-1 bg-canvas rounded-xl p-4">
                      <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-faint mb-1">
                        Framework
                      </p>
                      <p className="text-sm font-semibold text-ink">{confirmFw.name}</p>
                    </div>
                    <div className="flex-1 bg-canvas rounded-xl p-4">
                      <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-faint mb-1">
                        Controls
                      </p>
                      <p className="text-sm font-semibold text-ink">{confirmFw.controlCount}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 bg-[#FFFBEB] border border-[#FDE68A] rounded-lg px-3.5 py-3">
                    ...
                    <p className="text-xs text-[#92400E] leading-relaxed">
                      This will query the Microsoft 365 tenant in real-time. Please confirm you are
                      assessing the correct client.
                    </p>
                  </div>
                </div>
                ...
                <div className="px-6 pb-6 flex gap-3">
                  <button
                    onClick={() => setConfirmFw(null)}
                    className="flex-1 px-4 py-2.5 rounded-lg border border-border text-sm font-medium text-muted
