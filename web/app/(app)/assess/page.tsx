...
export default function AssessPage() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="mb-6">
        <h1 className="text-[22px] font-bold text-[#18181B] tracking-tight">Run Assessment</h1>
      ...
      {!loading && (
        <div className="mb-7">
          ...
          {clients.length === 0 ? (
            ...
            <div className="inline-flex items-center gap-2.5 bg-white border border-[#E9E5DD] rounded-xl px-4 py-2.5 shadow-card">
              <Building2 className="w-4 h-4 text-[#C4A96D]" />
              <span className="text-[13px] font-semibold text-[#18181B]">{selectedClient?.name}</span>
              <span className="text-[11px] font-mono text-[#9CA3AF]">{selectedClient?.tenantId.slice(0, 8)}…</span>
            </div>
          ) : (
            ...
            <div className="relative inline-block">
              <button
                ...
                className="inline-flex items-center gap-2.5 bg-white border border-[#E9E5DD] rounded-xl
                           px-4 py-2.5 shadow-card hover:border-[#D4CFC5] transition"
              >
                <Building2 className="w-4 h-4 text-[#C4A96D]" />
              ...
              {clientDropdown && (
                <div className="absolute top-full left-0 mt-1 z-20 bg-white border border-[#E9E5DD]
                                rounded-xl shadow-lg overflow-hidden min-w-[260px]">
                  {clients.map(c => (
                    <button
                      key={c.id}
                      onClick={() => { setSelectedClient(c); setClientDropdown(false) }}
                      className={`w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-[#F7F5F1] transition
                                  ${c.id === selectedClient?.id ? 'bg-[#F7F5F1]' : ''}`}
                    >
                      <Building2 className="w-3.5 h-3.5 text-[#9CA3AF] shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-medium text-[#18181B] truncate">{c.name}</p>
                        <p className="text-[11px] font-mono text-[#9CA3AF] truncate">{c.tenantId}</p>
                      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {frameworks.map(fw => {
              const accent      = FRAMEWORK_ACCENT[fw.id] ?? '#6B7280'
              ...
              return (
                <div
                  key={fw.id}
                  className={`relative bg-white border border-[#E9E5DD] rounded-xl overflow-hidden flex flex-col
                              shadow-card
                              ${disabled
                                ? 'opacity-50 cursor-not-allowed'
                                : 'cursor-pointer hover:border-[#D4CFC5] hover:shadow-card-hover transition-all'
                              }`}
                ...
                >
                  ...
                  <div className="pl-5 pr-5 pt-5 pb-4">
                    {!fw.implemented && (
                      <div className="absolute top-3.5 right-3.5">
                        <span className="text-[10px] font-medium bg-[#F3F4F6] text-[#9CA3AF] px-2 py-0.5 rounded-full flex items-center gap-1">
                    ...
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="font-semibold text-[#18181B] text-[14px] leading-snug">{fw.name}</h3>
                        {fw.version && (
                          <span className="text-[11px] text-[#9CA3AF] font-mono">{fw.version}</span>
                        )}
                      </div>
                    </div>
                    {fw.description && (
                      <p className="text-xs text-[#6B7280] leading-relaxed line-clamp-2">{fw.description}</p>
                    )}
                  </div>
                  ...
                  <div className="mt-auto px-5 pb-4 flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-xs text-[#9CA3AF]">
                      ...
                      <span><strong className="text-[#6B7280]">{fw.controlCount}</strong> controls</span>
                    </div>
                    {fw.implemented && (
                      <div className="flex items-center gap-0.5 text-xs font-semibold text-[#18181B]">
                        {isLaunching ? (
      ...
      {/* Info note */}
      <div className="mt-8 bg-white border border-[#E9E5DD] rounded-xl p-4 flex items-start gap-3 shadow-card">
        <Shield className="w-4 h-4 text-[#9CA3AF] shrink-0 mt-0.5" />
        <p className="text-xs text-[#6B7280] leading-relaxed">
          <strong className="text-[#18181B]">Assessment note:</strong>{' '}
