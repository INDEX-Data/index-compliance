  ...
  return (
    <div className="bg-surface rounded-xl border border-border overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-border-subtle">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-faint">
              {data
                ? `${data.total} policies with exclusions · ${data.withChanges} changed since last scan`
                : 'Scanning…'}
            </p>
          </div>
        </div>
        ...
        >
          <RefreshCw className={`w-3.5 h-3.5 text-faint ${loading ? 'animate-spin' : ''}`} />
        </button>
      </div>
      ...
      {data && data.policies.length === 0 && (
        <div className="flex items-center gap-3 px-5 py-4">
          ...
          <p className="text-[12px] text-muted">
            No Conditional Access policies have exclusion lists.
          </p>
        </div>
      )}
            ...
            <div key={p.policyId}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-faint">
                    {p.excludedUsers.length} user{p.excludedUsers.length !== 1 ? 's' : ''} ·{' '}
                    {p.excludedGroups.length} group{p.excludedGroups.length !== 1 ? 's' : ''}{' '}
                    excluded
                    {p.changed && (
                ...
                {p.justification ? (
                  <span className="text-[10px] text-[#0eb472] font-medium bg-[rgba(14,180,114,0.08)] px-2 py-0.5 rounded-full shrink-0">
                    Justified
                  </span>
                ) : (
                  <span className="text-[10px] text-[#f59e0b] font-medium bg-[rgba(245,158,11,0.08)] px-2 py-0.5 rounded-full shrink-0">
                    Needs review
                  </span>
                )}
                {expanded === p.policyId ? (
              ...
              {expanded === p.policyId && (
                <div className="px-5 pb-4 bg-canvas border-t border-[#f3f4f6]">
                  ...
                  <button
                    ...
                    className="mt-2 text-[12px] font-medium text-white px-4 py-2 rounded-lg transition-colors hover:bg-ink"
...
function AccessReviewPanel({ clientId }: { clientId: string }) {
  ...
  return (
    <div className="bg-surface rounded-xl border border-border overflow-hidden">
          ...
          <div>
            ...
            <p className="text-[11px] text-faint">
              {data?.supported === false
                ...
                : data
                  ? `${data.configured} configured · ${data.overdue} overdue`
                  : 'Loading…'}
            </p>
          </div>
        </div>
      ...
      {data?.supported === false && (
        <div className="px-5 py-4 text-[12px] text-faint">{data.message}</div>
      )}
      ...
      {data?.supported && data.definitions.length === 0 && (
        <div className="flex items-center gap-3 px-5 py-4">
          ...
          <p className="text-[12px] text-muted">
            No access reviews are configured. CMMC AC.3.012 requires periodic access reviews.
          </p>
        </div>
      )}
      ...
      {data?.supported && data.definitions.length > 0 && (
        <>
          ...
          <div
            className="flex items-center justify-between px-5 py-3 cursor-pointer hover:bg-canvas transition-colors"
            onClick={() => setExpanded((v) => !v)}
          >
            <span className="text-[12px] font-medium text-muted">View all reviews</span>
            {expanded ? (
  ...
  if (clients.length === 0)
    return (
      <div className="p-8 max-w-3xl text-center mt-12">
        ...
        <p className="text-[14px] font-semibold text-ink mb-1">No clients yet</p>
        <p className="text-[12px] text-faint">Add a client to start viewing security insights.</p>
      </div>
    )
    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-[14px] text-faint mt-1.5">
          Live compliance signals pulled from Microsoft Graph.
        </p>
      </div>
