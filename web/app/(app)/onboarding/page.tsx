...
export default function OnboardingPage() {
  ...
  if (checking) {
    return (
      <div className="flex items-center justify-center h-full">
        <ShieldCheck className="w-7 h-7 text-[#D4CFC5] animate-pulse" />
      </div>
    )
  }
  ...
  return (
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-12">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold border-2 transition-colors
                    ${i < step  ? 'bg-[#18181B] border-[#18181B] text-white'
                    : i === step ? 'border-[#18181B] text-[#18181B] bg-white'
                    :              'border-[#D4CFC5] text-[#D4CFC5] bg-white'}`}
                >
                  {i < step ? '✓' : i + 1}
                </div>
                <span className={`text-[12px] font-medium hidden sm:block transition-colors
                  ${i <= step ? 'text-[#374151]' : 'text-[#C4BFB5]'}`}>
                  {label}
                </span>
              </div>
              {/* Connector line */}
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-px mx-3 transition-colors ${i < step ? 'bg-[#18181B]' : 'bg-[#E9E5DD]'}`} />
              )}
            </div>
          ))}
        </div>
      ...
      {/* ── Card ── */}
      <div className="w-full max-w-lg bg-white border border-[#E9E5DD] rounded-2xl shadow-sm">

        {/* ══ Step 1: Welcome ══════════════════════════════════════════════ */}
        {step === 0 && (
          <div className="p-8">
            <div className="w-12 h-12 rounded-2xl bg-[#141412] flex items-center justify-center mb-6">
              <ShieldCheck className="w-6 h-6 text-white" />
            </div>

            <h1 className="text-2xl font-bold text-[#18181B] tracking-tight mb-2">
              Welcome to INDEX
            </h1>
            ...
            <div className="space-y-2.5 mb-8">
              {[
                ...
                <div
                  key={item.label}
                  className="flex items-start gap-3 bg-[#F7F5F1] rounded-xl px-4 py-3"
                >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-[#9CA3AF] mt-0.5">{item.hint}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 text-sm font-medium text-[#6B7280] hover:text-[#18181B] transition mb-8"
            >
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={() => setStep(1)}
                className="w-full bg-[#18181B] hover:bg-[#27272A] text-white font-semibold text-sm py-3 rounded-xl transition flex items-center justify-center gap-2"
              >
                Get Started <ChevronRight className="w-4 h-4" />
              </button>
              <button
                onClick={() => router.push('/dashboard')}
                className="text-sm text-[#9CA3AF] hover:text-[#6B7280] transition text-center py-1"
              >
        ...
        {step === 1 && (
          <div className="p-8">
            <h2 className="text-xl font-bold text-[#18181B] tracking-tight mb-1">
            ...
            <div className="space-y-4 mb-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-[#E9E5DD] rounded-lg px-3.5 py-2.5 text-sm text-[#18181B] focus:outline-none focus:ring-2 focus:ring-[#18181B]/20 placeholder-[#C4BFB5]"
                />
              </div>
              ...
              <div>
                <label className="block text-[12px] font-semibold text-[#374151] mb-1.5 uppercase tracking-wide">
                  Tenant ID
                  <span className="text-[#9CA3AF] font-normal ml-2 normal-case">Azure AD → Properties</span>
                </label>
                <input
                  type="text"
                  value={tenantId}
                  ...
                  className="w-full border border-[#E9E5DD] rounded-lg px-3.5 py-2.5 text-sm font-mono text-[#18181B] focus:outline-none focus:ring-2 focus:ring-[#18181B]/20 placeholder-[#C4BFB5]"
                />
              </div>
              ...
              <div>
                <label className="block text-[12px] font-semibold text-[#374151] mb-1.5 uppercase tracking-wide">
                  Client ID
                  <span className="text-[#9CA3AF] font-normal ml-2 normal-case">App Registration → Overview</span>
                </label>
                <input
                  type="text"
                  value={clientId}
              ...
              <div>
                <label className="block text-[12px] font-semibold text-[#374151] mb-1.5 uppercase tracking-wide">
                  Client Secret
                  <span className="text-[#9CA3AF] font-normal ml-2 normal-case">Certificates &amp; Secrets</span>
                </label>
                <div className="relative">
                  <input
                    ...
                    className="w-full border border-[#E9E5DD] rounded-lg px-3.5 py-2.5 pr-10 text-sm font-mono text-[#18181B] focus:outline-none focus:ring-2 focus:ring-[#18181B]/20 placeholder-[#C4BFB5]"
                  />
                  <button
                    type="button"
            ...
            <button
              ...
              className="w-full border border-[#E9E5DD] hover:bg-[#F7F5F1] text-[#374151] font-medium text-sm py-2.5 rounded-xl transition mb-6 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => setStep(0)}
                className="flex-1 border border-[#E9E5DD] hover:bg-[#F7F5F1] text-[#374151] font-medium text-sm py-2.5 rounded-xl transition"
              >
                ← Back
              </button>
              <button
                ...
                className="flex-[2] bg-[#18181B] hover:bg-[#27272A] text-white font-semibold text-sm py-2.5 rounded-xl transition disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
            ...
            <div className="space-y-2.5 mb-8">
              {frameworks.length === 0 ? (
                <div className="flex items-center justify-center py-8">
                  <Loader2 className="w-5 h-5 animate-spin text-[#D4CFC5]" />
                </div>
              ) : (
                frameworks.map(fw => (
                  <button
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-semibold text-[#18181B]">{fw.name}</p>
                      {fw.description && (
                        <p className="text-[11px] text-[#9CA3AF] mt-0.5 truncate">{fw.description}</p>
                      )}
                    </div>

                    {/* Control count badge */}
                    <span className="text-[11px] text-[#C4BFB5] font-mono shrink-0">
      ...
      <p className="text-[11px] text-[#C4BFB5] mt-5 font-mono">
