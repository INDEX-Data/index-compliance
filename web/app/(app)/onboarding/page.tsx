...
export default function OnboardingPage() {
  ...
  if (checking) {
    return (
      <div className="flex items-center justify-center h-full">
        <ShieldCheck className="w-7 h-7 text-[#CBD5E1] animate-pulse" />
      </div>
    )
  }
  ...
  return (
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-12">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold border-2 transition-colors
                    ${i < step  ? 'bg-[#0F172A] border-[#0F172A] text-white'
                    : i === step ? 'border-[#0F172A] text-[#0F172A] bg-white'
                    :              'border-[#CBD5E1] text-[#CBD5E1] bg-white'}`}
                >
                  {i < step ? '✓' : i + 1}
                </div>
                <span className={`text-[12px] font-medium hidden sm:block transition-colors
                  ${i <= step ? 'text-[#334155]' : 'text-[#B0BEC5]'}`}>
                  {label}
                </span>
              </div>
              {/* Connector line */}
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-px mx-3 transition-colors ${i < step ? 'bg-[#0F172A]' : 'bg-[#E2E8F0]'}`} />
              )}
            </div>
          ))}
        </div>
      ...
      {/* ── Card ── */}
      <div className="w-full max-w-lg bg-white border border-[#E2E8F0] rounded-2xl shadow-sm">

        {/* ══ Step 1: Welcome ══════════════════════════════════════════════ */}
        {step === 0 && (
          <div className="p-8">
            <div className="w-12 h-12 rounded-2xl bg-[#0B1829] flex items-center justify-center mb-6">
            ...
            <div className="space-y-2.5 mb-8">
              {[
                ...
                <div
                  key={item.label}
                  className="flex items-start gap-3 bg-[#F8FAFC] rounded-xl px-4 py-3"
                >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-[#94A3B8] mt-0.5">{item.hint}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 text-sm font-medium text-[#64748B] hover:text-[#0F172A] transition mb-8"
            >
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={() => setStep(1)}
                className="w-full bg-[#0F172A] hover:bg-[#1E293B] text-white font-semibold text-sm py-3 rounded-xl transition flex items-center justify-center gap-2"
              >
                Get Started <ChevronRight className="w-4 h-4" />
              </button>
              <button
                onClick={() => router.push('/dashboard')}
                className="text-sm text-[#94A3B8] hover:text-[#64748B] transition text-center py-1"
              >
        ...
        {step === 1 && (
          <div className="p-8">
            ...
            <div className="space-y-4 mb-5">
              ...
              {/* Client Name */}
              <div>
                <label className="block text-[12px] font-semibold text-[#334155] mb-1.5 uppercase tracking-wide">
                  Client Name
                </label>
                <input
                  type="text"
                  ...
                  className="w-full border border-[#E2E8F0] rounded-lg px-3.5 py-2.5 text-sm text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-[#0F172A]/20 placeholder-[#B0BEC5]"
                />
              </div>
              ...
              <div>
                <label className="block text-[12px] font-semibold text-[#334155] mb-1.5 uppercase tracking-wide">
                  Tenant ID
                  <span className="text-[#94A3B8] font-normal ml-2 normal-case">Azure AD → Properties</span>
                </label>
                <input
                  type="text"
                  value={tenantId}
                  ...
                  className="w-full border border-[#E2E8F0] rounded-lg px-3.5 py-2.5 text-sm font-mono text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-[#0F172A]/20 placeholder-[#B0BEC5]"
                />
              </div>
              ...
              <div>
                <label className="block text-[12px] font-semibold text-[#334155] mb-1.5 uppercase tracking-wide">
                  Client ID
                  <span className="text-[#94A3B8] font-normal ml-2 normal-case">App Registration → Overview</span>
                </label>
                <input
                  type="text"
                  value={clientId}
              ...
              <div>
                <label className="block text-[12px] font-semibold text-[#334155] mb-1.5 uppercase tracking-wide">
                  Client Secret
                  <span className="text-[#94A3B8] font-normal ml-2 normal-case">Certificates &amp; Secrets</span>
                </label>
                <div className="relative">
                  <input
                    ...
                    className="w-full border border-[#E2E8F0] rounded-lg px-3.5 py-2.5 pr-10 text-sm font-mono text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-[#0F172A]/20 placeholder-[#B0BEC5]"
                  />
                  <button
                    type="button"
            ...
            <button
              ...
              className="w-full border border-[#E2E8F0] hover:bg-[#F8FAFC] text-[#334155] font-medium text-sm py-2.5 rounded-xl transition mb-6 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => setStep(0)}
                className="flex-1 border border-[#E2E8F0] hover:bg-[#F8FAFC] text-[#334155] font-medium text-sm py-2.5 rounded-xl transition"
              >
                ← Back
              </button>
              <button
                ...
                className="flex-[2] bg-[#0F172A] hover:bg-[#1E293B] text-white font-semibold text-sm py-2.5 rounded-xl transition disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                ...
                frameworks.map(fw => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-semibold text-[#0F172A]">{fw.name}</p>
                      {fw.description && (
                        <p className="text-[11px] text-[#94A3B8] mt-0.5 truncate">{fw.description}</p>
                      )}
                    </div>

                    {/* Control count badge */}
                    <span className="text-[11px] text-[#B0BEC5] font-mono shrink-0">
      ...
      <p className="text-[11px] text-[#B0BEC5] mt-5 font-mono">
