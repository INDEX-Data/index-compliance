      ...
      <div className="bg-white rounded-2xl border border-border shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border-subtle">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[14px] font-bold text-ink">Invite a Client</span>
          </div>
          <button onClick={onClose} className="text-faint hover:text-muted transition">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-ink mb-1.5 uppercase tracking-wide">
                Email <span className="font-normal normal-case text-faint">(optional)</span>
              </label>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-faint" />
                <input
                  type="email"
                  value={email}
          ...
          {loadingList ? (
            ...
            <p className="text-xs text-faint text-center py-3">No invitations yet.</p>
          ) : (
              ...
              {invitations.map((inv) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-ink truncate">{inv.clientName}</p>
                    {inv.email && <p className="text-[10px] text-faint truncate">{inv.email}</p>}
                  </div>
                  {statusBadge(inv)}
                  {inv.status === 'pending' && (
                    <button
                      ...
                      className="text-[10px] font-medium text-faint hover:text-[#B91C1C] transition shrink-0"
                    >
                      {revoking === inv.id ? (
      ...
      <div className="bg-white rounded-2xl border border-border shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border-subtle">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[14px] font-bold text-ink">Add New Client</span>
          </div>
          <button onClick={onClose} className="text-faint hover:text-muted transition">
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="rounded-lg border border-border overflow-hidden">
            ...
            {guideOpen && (
              <div className="px-4 pt-4 pb-3 border-t border-border space-y-3.5">
                {[
                  {
                    step: '1',
                ...
                <a
                  ...
                  className="flex items-center gap-1.5 text-xs text-muted hover:text-ink transition pt-1"
                >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-[14px] font-semibold text-ink truncate">{client.name}</h3>
          <p className="text-[11px] text-faint font-mono truncate mt-0.5">{client.tenantId}</p>
        </div>
        ...
        {lastReport && (
          <div className="text-right shrink-0">
            ...
            <p className="text-[10px] text-faint">{lastReport.frameworkId}</p>
          </div>
        )}
        ...
        <div className="flex items-center gap-1.5 shrink-0" onClick={(e) => e.preventDefault()}>
          ...
          {!confirming ? (
            <button
              ...
              className="p-1.5 rounded-lg text-faint hover:text-[#f25757] hover:bg-[#FEF2F2] transition-colors"
            >
            ...
            <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
              ...
              <button
                ...
                className="px-2.5 py-1 rounded-lg border border-border text-[11px] text-muted transition-colors"
              >
                Cancel
              </button>
            </div>
    ...
    <div className="p-8 max-w-5xl">
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-sm text-faint mb-6 max-w-xs mx-auto">
          ...
          <div className="bg-[#fafafa] border border-border rounded-xl px-5 py-4 mb-6 max-w-sm mx-auto text-left">
            ...
            <ul className="space-y-2.5">
              {[
                ...
                <li key={item.label} className="flex items-start gap-2.5">
                  <div className="w-4 h-4 rounded border-2 border-border-strong shrink-0 mt-0.5" />
                  <div>
                    <span className="text-sm text-ink font-medium">{item.label}</span>
                    <span className="text-xs text-faint ml-1.5">{item.hint}</span>
                  </div>
                </li>
              ))}
            </ul>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-3.5 flex items-center gap-1.5 text-xs text-muted hover:text-ink transition"
            >
              <ExternalLink className="w-3 h-3" />
              Open Azure Portal
            </a>
