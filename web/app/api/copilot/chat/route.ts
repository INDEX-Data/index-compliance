...
import { executeIntegrationQuery, isQueryable, getQueryTypes, describeIntegrationQuery } from '@/lib/integration-queries'
import { getDefenderToken, executeDefenderQuery, describeDefenderQuery, DEFENDER_QUERY_TYPES } from '@/lib/defender-queries'
import { getManagementActivityToken, executeAuditQuery, describeAuditQuery, AUDIT_QUERY_TYPES } from '@/lib/management-activity-queries'
import { checkAllPermissions, permissionsToPromptSection, type PermissionsReport } from '@/lib/permissions-check'
...
async function graphQuery(token: string, endpoint: string, apiVersion: string = 'v1.0'): Promise<any> {
  const base = apiVersion === 'beta'
...
async function graphPost(token: string, endpoint: string, body: any, apiVersion: string = 'beta'): Promise<any> {
  const base = apiVersion === 'beta'
    ? 'https://graph.microsoft.com/beta'
    : 'https://graph.microsoft.com/v1.0'
  const url = `${base}${endpoint}`
...
You can use OData query params like $select, $filter, $top, $orderby, $count, $expand directly in the endpoint string. Use beta API version for endpoints that require it (e.g., security, some deviceManagement).`,
    input_schema: {
      ...
      properties: {
        endpoint: {
          type: 'string',
          description: 'Graph API endpoint path, e.g. "/users?$select=displayName,mail,accountEnabled&$top=10"',
        },
        api_version: {
          type: 'string',
          enum: ['v1.0', 'beta'],
          description: 'API version. Default v1.0. Use beta for security, some deviceManagement endpoints.',
        },
        method: {
          type: 'string',
          enum: ['GET', 'POST'],
        ...
        post_body: {
          type: 'object',
          description: 'Body for POST requests. E.g., {"Query": "DeviceProcessEvents | take 10"} for hunting.',
        },
      },
      required: ['endpoint'],
    },
  },
  {
    ...
    description: 'Get all compliance assessment reports for the active client. Returns framework IDs, scores, dates, and control-level results. Use this to answer questions about past assessments, compliance scores, failing controls, trends over time, etc.',
    input_schema: {
      type: 'object' as const,
      properties: {
        framework_id: {
          type: 'string',
          description: 'Optional: filter to a specific framework (e.g., "cmmc-l2"). Omit to get all frameworks.',
        },
        limit: {
          type: 'number',
...
For advanced_hunting, pass a KQL query in params.kql. Example: "DeviceProcessEvents | where FileName == 'powershell.exe' | take 20"`,
    input_schema: {
      ...
      properties: {
        ...
        params: {
          type: 'object',
          description: 'Query parameters. For advanced_hunting: {kql: "..."}. For others: {limit?: number, filter?: string}.',
          properties: {
    ...
    input_schema: {
      ...
      properties: {
        ...
        params: {
          ...
          properties: {
            start_time: { type: 'string', description: 'ISO 8601 start time. Default: 24 hours ago.' },
...
function buildIntegrationTool(connectedPlatforms: { platform: string; name?: string }[]): Anthropic.Tool | null {
  const queryable = connectedPlatforms.filter(p => isQueryable(p.platform))
  if (queryable.length === 0) return null

  const platformDocs = queryable.map(p => {
...
Each query_type accepts different params — use reasonable defaults. For Jira search_issues, pass {jql: "..."} for JQL queries. For ServiceNow, pass {query: "..."} for encoded queries. For Splunk search, pass {query: "search ..."}.`,
    input_schema: {
      ...
      properties: {
        ...
        params: {
          type: 'object',
          description: 'Query-specific parameters (e.g., jql, query, limit, issueKey, boardId, table, etc.)',
        },
      },
...
export async function POST(request: Request) {
  try {
    ...
    const { messages = [], context = {}, conversationId: reqConversationId } = body as {
    ...
    const [allClientsResult, reportsResult, integrationsResult, currentReportResult, permissionsResult] = await Promise.all([
      // 1. All clients
      admin.from('clients').select('id, name, tenant_id, added_at').eq('user_id', user.id).order('added_at', { ascending: false }),
      // 2. Recent reports
      clientRow
        ? admin.from('reports').select('id, framework_id, generated_at, data').eq('client_id', clientRow.id).eq('user_id', user.id).order('generated_at', { ascending: false }).limit(5)
        : Promise.resolve({ data: null }),
      // 3. Connected integrations
      clientRow
        ? admin.from('client_integrations').select('id, platform, config, status').eq('client_id', clientRow.id).eq('status', 'connected')
        : Promise.resolve({ data: null }),
      // 4. Current report (if viewing assessment)
      context.reportId
        ? admin.from('reports').select('id, framework_id, generated_at, data').eq('id', context.reportId).eq('user_id', user.id).single()
        : Promise.resolve({ data: null }),
      // 5. API permissions (cached 10 min)
      clientRow
        ? getCachedPermissions(decryptIfNeeded(clientRow.tenant_id), decryptIfNeeded(clientRow.client_id), decryptIfNeeded(clientRow.client_secret)).catch(() => null)
        : Promise.resolve(null),
    ])

    const allClients = allClientsResult.data
    const clientsList = allClients && allClients.length > 0
      ? allClients.map(c =>
    ...
    if (reports && reports.length > 0) {
      reportsSection = reports.map((r: any) => {
    ...
    let integrations: Array<{ id: string; platform: string; config: Record<string, string>; status: string }> = []
    ...
    if (intRows && intRows.length > 0) {
      ...
      integrationsSection = intRows.map((i: any) => {
        const types = isQueryable(i.platform) ? getQueryTypes(i.platform).join(', ') : 'notifications only'
    ...
    if (currentReport) {
      ...
      const failingControls = controls
        ...
        .slice(0, 10)
        .map((c: any) => `  - \`${c.controlId}\` ${c.controlTitle || c.title}: ${(c.findings || []).slice(0, 2).join('; ')}`)
        .join('\n')
...
${d?.summary?.topFindings ? `- **Key Findings**: ${d.summary.topFindings.slice(0, 3).join('; ')}` : ''}`
    ...
    const permissionsReport: PermissionsReport | null = permissionsResult as PermissionsReport | null
    const permissionsSection = permissionsReport ? permissionsToPromptSection(permissionsReport) : 'Unable to check API permissions.'
...
- For KQL advanced hunting, prefer query_defender with advanced_hunting query type — it supports richer device/network/email tables.`
    ...
    function describeToolCall(toolName: string, input: any): string {
      if (toolName === 'query_graph_api') {
        ...
        if (ep.includes('/security/runHuntingQuery')) return 'Running advanced hunting query via Graph'
      ...
      if (toolName === 'get_assessment_reports') {
        if (input.framework_id) return `Pulling ${input.framework_id.toUpperCase()} assessment reports`
    ...
    const readable = new ReadableStream({
      async start(controller) {
        ...
        async function executeTool(toolCall: { id: string; name: string; input: any }): Promise<{ id: string; result: string; desc: string; error?: boolean }> {
          ...
          try {
            ...
            if (toolCall.name === 'query_graph_api') {
              ...
              if (input.method === 'POST' && input.post_body) {
                data = await graphPost(token, input.endpoint, input.post_body, input.api_version || 'beta')
              } else {
              ...
              sendStatus('tool_done', typeof count === 'number' ? `${desc} — ${count} result${count !== 1 ? 's' : ''}` : `${desc} — done`)

            } else if (toolCall.name === 'query_defender') {
              if (!clientRow) throw new Error('No client connected')
              const defToken = await getDefenderToken(decryptIfNeeded(clientRow.tenant_id), decryptIfNeeded(clientRow.client_id), decryptIfNeeded(clientRow.client_secret))
              const defResult = await executeDefenderQuery(defToken, input.query_type, input.params || {})
              result = truncateResult(defResult.data)
              sendStatus('tool_done', `${desc} — ${defResult.summary}`)

            } else if (toolCall.name === 'query_audit_log') {
              if (!clientRow) throw new Error('No client connected')
              const auditToken = await getManagementActivityToken(decryptIfNeeded(clientRow.tenant_id), decryptIfNeeded(clientRow.client_id), decryptIfNeeded(clientRow.client_secret))
              const auditResult = await executeAuditQuery(auditToken, decryptIfNeeded(clientRow.tenant_id), input.query_type, input.params || {})
              ...
              let query = admin
                .from('reports').select('id, framework_id, generated_at, data')
                .eq('user_id', user!.id).eq('client_id', clientRow.id)
                .order('generated_at', { ascending: false }).limit(input.limit || 10)
              ...
              const formatted = (rpts || []).map((r: any) => {
                const d = r.data as any
                const controls = d?.controlAssessments || d?.controls || []
                return { reportId: r.id, frameworkId: r.framework_id, generatedAt: r.generated_at,
                  clientName: d?.clientName || d?.tenantDisplayName, summary: d?.summary, dibcacSummary: d?.dibcacSummary,
                  controls: controls.map((c: any) => ({ controlId: c.controlId, title: c.controlTitle || c.title, status: c.status, findings: c.findings?.slice(0, 3), recommendations: c.recommendations?.slice(0, 2) })),
                }
              })
              result = truncateResult(formatted)
              sendStatus('tool_done', `${desc} — ${(rpts || []).length} report${(rpts || []).length !== 1 ? 's' : ''}`)
              ...
              for (const [k, v] of Object.entries(intConfig.config)) { decryptedConfig[k] = decryptIfNeeded(v as string) }
              const data: any = await executeIntegrationQuery(input.platform, decryptedConfig, input.query_type, input.params || {})
              result = truncateResult(data)
            ...
            sendStatus('tool_error', `${desc} — failed: ${err.message?.slice(0, 100) || 'unknown error'}`)
