  ...
  for (const query of control.evidenceQueries) {
    ...
    try {
      ...
      if (Array.isArray(response.value)) {
        ...
        const meaningfulKeys = (Object.keys(flat) as string[])
...
const evaluators: Record<string, EvaluatorFn> = {
  ...
  evaluate_mfa_enforcement: (_control, evidence) => {
    ...
    if (hasMfaPolicy && caEvidence.recordCount > 0) {
      return {
        status: "pass",
        findings: [`Found ${caEvidence.recordCount} Conditional Access policies, MFA grant controls detected`],
        recommendations: [],
      };
    }
  ...
  evaluate_sensitivity_labels: (_control, evidence) => {
    ...
    if (!auditEvidence?.success && !scoreEvidence?.success) {
      return {
        status: "not_assessed",
        findings: ["Unable to collect information protection evidence (audit logs and secure score unavailable)"],
        recommendations: ["Grant AuditLog.Read.All and SecurityEvents.Read.All to the app registration"],
      };
    }
  ...
  evaluate_risk_assessment: (_control, evidence) => {
    ...
    const riskCount = (riskEvidence?.success ? riskEvidence.recordCount : 0)
    ...
    if (riskEvidence?.success || riskyUsersEvidence?.success) {
      if (riskCount > 0) {
        ...
        recommendations.push("Investigate and remediate active identity risk detections in Entra ID Protection");
        recommendations.push("Enable risk-based Conditional Access policies to auto-remediate risky sign-ins");
  ...
  evaluate_security_monitoring: (_control, evidence) => {
    ...
    const anySuccess = [alertEvidence, riskEvidence, riskyUsersEvidence, signInEvidence]
      .some((e) => e?.success);
    ...
    if (alertEvidence?.success) {
      if (alertEvidence.recordCount > 0) {
        findings.push(`${alertEvidence.recordCount} security alert(s) found — review and triage required`);
        recommendations.push("Triage and resolve open security alerts in Microsoft Defender / Sentinel");
        hasOpenAlerts = true;
      } else {
    ...
    if (signInEvidence?.success && signInEvidence.recordCount > 0) {
      findings.push(`Sign-in activity is being logged (${signInEvidence.recordCount} recent records)`);
    }
    ...
    if (riskEvidence?.success || riskyUsersEvidence?.success) {
      if (riskCount > 0) {
        ...
        recommendations.push("Remediate risky users via Entra ID Protection (require MFA re-registration or reset password)");
        hasOpenAlerts = true;
      } else {
    ...
    recommendations.push("Configure alert notification emails/webhooks to your SOC or security team");
  ...
  evaluate_guest_access: (_control, evidence) => {
    ...
    if (guestCount > 50) {
      findings.push(`Guest account count (${guestCount}) is elevated — stale accounts may be present`);
    ...
    if (isOpen) {
      ...
      recommendations.push("Change External Collaboration Settings to restrict invitations to admins only");
      recommendations.push("Enable quarterly access reviews for all guest users in Entra ID Governance");
    ...
    if (isModerate) {
      ...
      recommendations.push("Restrict guest invitations to 'Admins and guest inviters' in External Collaboration Settings");
    }
