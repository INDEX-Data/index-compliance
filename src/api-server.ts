...
import { getFrameworkControls, getFramework, listFrameworks } from './data/framework-registry.js'
import { DIBCAC_OBJECTIVES, type DIBCACObjective } from './data/dibcac-objectives.js'
import type {
  ComplianceReport,
  ComplianceSummary,
  ControlAssessment,
...
async function initDB() {
  ...
  try {
    ...
    console.warn(
      '[INDEX] Database     →  ⚠ failed to connect, falling back to file storage:',
...
async function requireAuth(req: AuthedRequest, res: Response, next: NextFunction) {
  ...
  const token =
    req.headers.authorization?.replace('Bearer ', '') ?? (req.query.token as string | undefined)
  ...
  try {
    ...
    console.warn(
      '[AUTH] verifyToken attempt 1 failed:',
...
async function listReportMetas(userId: string = DEV_USER_ID) {
  ...
  try {
    ...
    return readdirSync(REPORTS_DIR)
      ...
      .sort(
        (a, b) =>
          new Date((b as any).generatedAt).getTime() - new Date((a as any).generatedAt).getTime()
      )
  } catch {
    return []
  }
...
app.post('/api/config/test', async (req, res) => {
  ...
  const probes: { path: string; extract: (d: any) => { name?: string; domain?: string } }[] = [
    {
      ...
      extract: (d) => {
        ...
        return {
          name: o?.displayName,
          domain: o?.verifiedDomains?.find((v: any) => v.isDefault)?.name,
        }
      },
    },
    {
  ...
  const displayName =
    tenantName ?? (domain ? `Tenant (${domain})` : `Tenant ${testClient.tenantId}`)
...
app.post('/api/clients', async (req: AuthedRequest, res) => {
  ...
  if (!name || !tenantId || !clientId || !clientSecret)
    return void res
      .status(400)
      .json({ error: 'name, tenantId, clientId, and clientSecret are required' })
...
app.get('/api/clients/:id/integrations', async (req, res) => {
  ...
  const rows = await (db as any).select().from(intTable).where(eq(intTable.clientId, req.params.id))
...
app.put('/api/clients/:id/integrations/:platform', async (req: AuthedRequest, res) => {
  ...
  const clientRows = await (db as any)
    .select()
    .from(clientTable)
...
app.get('/api/team/members', async (req: AuthedRequest, res) => {
  ...
  const rows = await (db as any).select().from(membTable).where(eq(membTable.ownerId, userId))
...
app.post('/api/team/join/:token/accept', async (req: AuthedRequest, res) => {
  ...
  if (new Date(inv.expiresAt) < new Date())
    return void res.status(410).json({ error: 'This invite has expired' })
  if (inv.ownerId === memberId)
    return void res.status(400).json({ error: 'You cannot accept your own invite' })
  ...
  await (db as any).update(invTable).set({ status: 'accepted' }).where(eq(invTable.id, inv.id))
...
app.get('/api/onboard/:token/integrations', async (req, res) => {
  ...
  const rows = await (db as any).select().from(intTable).where(eq(intTable.clientId, inv.clientId))
...
app.get('/api/frameworks/:id', (req, res) => {
  ...
  res.json({
    ...
    implemented: fw.controls.length > 0,
  })
})
...
app.get('/api/reports/:id/export/word', async (req: AuthedRequest, res) => {
  ...
  if (!apiKey) {
    return void res.status(500).json({
      error:
        'Anthropic API key is not configured on this server. Set ANTHROPIC_API_KEY in the environment.',
...
function buildObjectiveSummary(statuses: ObjectiveStatus[]): DIBCACObjectiveSummary {
  ...
  const coveragePercentage =
    nonPhysical > 0 ? Math.round(((met + partiallyMet * 0.5) / nonPhysical) * 100) : 0
...
app.post('/api/reports/:id/objectives/reset', async (req: AuthedRequest, res) => {
  ...
  if (report.frameworkId !== 'CMMC_L2')
    return void res
      .status(400)
      .json({ error: 'DIBCAC objectives only apply to CMMC L2 assessments' })
...
app.get('/api/reports/:id/objectives/export/csv', async (req: AuthedRequest, res) => {
  ...
  const HEADER =
    'Objective_Number,Objective_Text,Objective_Satisfied,Objective_Other_Than_Satisfied,Documents_Examined_Details,SME_Interviewed_Names,Validation_Text,Notes,Standard'

  const rows = DIBCAC_OBJECTIVES.map((obj) => {
    const st = statusMap.get(obj.objectiveId)
    const satisfied = st?.status === 'met' ? 'TRUE' : 'FALSE'
    const otherThanSat =
      st?.status === 'not_met' || st?.status === 'requires_physical' ? 'TRUE' : 'FALSE'
...
app.get('/api/reports/:id/export/opa', async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    const failed = report.controlAssessments.filter(
        ...
        new TableRow({
          children: [
            new TableCell({
              ...
              children: [
                new Paragraph({
                  children: [new TextRun({ text: c.controlId, size: 18, font: 'Arial' })],
                }),
              ],
            }),
            new TableCell({
              ...
              children: [
                new Paragraph({
                  children: [
                    new TextRun({ text: c.controlTitle ?? c.controlId, size: 18, font: 'Arial' }),
                  ],
                }),
              ],
            }),
            ...
            new TableCell({
              ...
              children: [
                new Paragraph({
                  children: [
                    new TextRun({
                      text: c.recommendations?.[0] ?? 'Remediation required — see full report.',
                      size: 18,
                      font: 'Arial',
                    }),
                  ],
            ...
            new TableCell({
              ...
              children: [
                new Paragraph({
                  children: [new TextRun({ text: 'Open', size: 18, font: 'Arial' })],
                }),
              ],
            }),
            new TableCell({
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const openItemsBlock: any[] =
      failed.length === 0
        ? [
            new Paragraph({
              children: [
                new TextRun({
                  text: 'All controls are satisfied. No open items.',
                  size: 22,
                  font: 'Arial',
                  color: '16A34A',
                }),
      ...
      sections: [
        {
          ...
          children: [
            new Paragraph({
              ...
              children: [
                new TextRun({
                  text: 'Operational Plan of Action (OPA)',
                  bold: true,
                  size: 40,
                  font: 'Arial',
                }),
            ...
            new Paragraph({
              children: [
                ...
                new TextRun({
                  text: `${report.summary.compliancePercentage}%`,
                  size: 22,
                  font: 'Arial',
                }),
              ],
            ...
            new Paragraph({
              children: [
                new TextRun({
                  text: `Total: ${report.controlAssessments.length}   `,
                  size: 22,
                  font: 'Arial',
                }),
                new TextRun({
                ...
                new TextRun({
                  text: `Partial: ${report.summary.partial}   `,
                  size: 22,
                  font: 'Arial',
                  color: 'D97706',
                }),
                ...
                new TextRun({
                  text: `Manual: ${report.summary.manualRequired ?? 0}   `,
                  size: 22,
                  font: 'Arial',
                  color: '6D28D9',
                }),
            ...
            new Paragraph({
              children: [
                new TextRun({
                  text: `Automated coverage: ${report.summary.automatedCoverage ?? 0}%   `,
                  size: 20,
                  font: 'Arial',
                  color: '505967',
                }),
                new TextRun({
                  text: `Collection health: ${report.summary.collectionHealth ?? 100}%`,
                  size: 20,
                  font: 'Arial',
                  color: report.summary.lowCoverageWarning ? 'DC2626' : '505967',
                }),
                ...(report.summary.lowCoverageWarning
                  ? [
                      new TextRun({
                        text: '   ⚠ Score computed on incomplete data — some automated checks failed to collect this run.',
                        size: 20,
                        font: 'Arial',
                        italics: true,
                        color: 'DC2626',
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="OPA_${safe}_${tenant}_${report.reportId}.docx"`
    )
    res.end(buffer)
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
...
app.get('/api/reports/:id/export/zip', async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    for (const control of report.controlAssessments) {
      const family = (control.family ?? control.frameworkId ?? 'other').replace(
        /[^a-z0-9_\-]/gi,
        '_'
      )
      ...
      folder.file(
        ...
        [
          ...
          '',
          'Recommendations:',
          ...(control.recommendations?.map((r, i) => `  ${i + 1}. ${r}`) ?? [
    ...
    const manifest = {
      ...
      notAssessed: report.summary.notAssessed,
      manualRequired: report.summary.manualRequired ?? 0,
      compliancePercentage: report.summary.compliancePercentage,
      automatedCoverage: report.summary.automatedCoverage ?? null,
      collectionHealth: report.summary.collectionHealth ?? null,
    }
    const manifestStr = JSON.stringify(manifest, null, 2)
    zip.file('manifest.json', manifestStr)
    zip.file(
      'MANIFEST.sha256',
      `${createHash('sha256').update(manifestStr).digest('hex')}  manifest.json\n`
    )
    zip.file(
      'README.txt',
      [
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="Evidence_${safe}_${tenant}_${report.reportId}.zip"`
    )
    res.end(buffer)
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
...
async function deleteEvidenceFile(reportId: string, fileId: string): Promise<boolean> {
  if (db && dbSchema && drizzleOps) {
    ...
    await (db as any)
      ...
      .where(and(eq(evidenceFiles.id, fileId), eq(evidenceFiles.reportId, reportId)))
    return true
  }
  const p = evidenceFilesPath(reportId)
...
app.get(
  '/api/reports/:id/objectives/:objId/files/:fileId/download',
...
app.get('/api/reports/drift', async (req: AuthedRequest, res) => {
  ...
  const allReports = await (async () => {
    if (db && dbSchema && drizzleOps) {
      ...
      const rows = await (db as any)
        ...
        .from(reportsTable)
        .where(and(...conds))
        .orderBy(desc((reportsTable as any).generatedAt))
        .limit(2)
    ...
    try {
      ...
      const loaded = files
        .map((f: string) => {
          try {
            return JSON.parse(readFileSync(join(REPORTS_DIR, f), 'utf8')) as ComplianceReport
          } catch {
            return null
          }
        })
        .filter(
          (r: ComplianceReport | null): r is ComplianceReport =>
            r !== null && (r as any).userId === req.userId
        )
      return loaded
        .filter(
          (r: ComplianceReport) =>
            (!frameworkId || r.frameworkId === frameworkId) &&
            (!clientId || (r as any).clientId === clientId)
        )
        .sort(
  ...
  if (allReports.length < 2) {
    return void res.json({
      hasDrift: false,
      message: 'Need at least two assessments to detect drift.',
  ...
  for (const [id, curr] of latestMap) {
    ...
    const improved =
      (prev.status === 'fail' && curr.status !== 'fail') ||
      (prev.status === 'partial' && curr.status === 'pass')
    const degraded =
      (curr.status === 'fail' && prev.status !== 'fail') ||
      (curr.status === 'partial' && prev.status === 'pass')
    changed.push({
      controlId: id,
      controlName: curr.controlTitle ?? id,
      from: prev.status,
      to: curr.status,
      direction: improved ? 'improved' : degraded ? 'degraded' : 'changed',
    })
  }
  ...
  res.json({
    ...
    latestReport: {
      id: latest.reportId,
      score: latest.summary.compliancePercentage,
      generatedAt: latest.generatedAt,
    },
    previousReport: {
      id: previous.reportId,
      score: previous.summary.compliancePercentage,
...
app.get('/api/profile', async (req: AuthedRequest, res: Response) => {
  if (db && dbSchema) {
    ...
    const rows = await db
      ...
      .where(eq(dbSchema.userProfiles.userId, req.userId ?? 'dev'))
      .limit(1)
...
app.put('/api/profile', async (req: AuthedRequest, res: Response) => {
  ...
  if (!companyName || !accountType)
    return void res.status(400).json({ error: 'companyName and accountType are required' })
...
app.put('/api/clients/:id/notes', async (req: AuthedRequest, res: Response) => {
  ...
  if (db && dbSchema) {
    const { eq, and } = drizzleOps!
    await db
      .update(dbSchema.clients)
      .set({ notes })
      .where(
        and(
          eq(dbSchema.clients.id, req.params.id),
          eq(dbSchema.clients.userId, req.userId ?? 'dev')
        )
      )
  }
  res.json({ ok: true })
...
app.get('/api/clients/:id/scoping', async (req: AuthedRequest, res: Response) => {
  ...
  if (db && dbSchema) {
    ...
    const rows = await db
      .select()
      .from(dbSchema.clientScoping)
      .where(eq(dbSchema.clientScoping.clientId, id))
      .limit(1)
...
app.put('/api/clients/:id/scoping', async (req: AuthedRequest, res: Response) => {
  ...
  if (db && dbSchema) {
    await db
      .insert(dbSchema.clientScoping)
      .values({ clientId: id, userId: req.userId ?? 'dev', scoping })
      .onConflictDoUpdate({
        target: dbSchema.clientScoping.clientId,
        set: { scoping, updatedAt: new Date() },
      })
  }
...
app.get('/api/clients/:id/ca-exclusions', async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const raw = await gc.query<any>('/policies/conditionalAccessPolicies', {
      select: ['id', 'displayName', 'conditions', 'state'],
      top: 100,
    })
    ...
    if (db && dbSchema) {
      ...
      const rows = await db
        .select()
        .from(dbSchema.caExclusionSnapshots)
    ...
    const results = policies.map((p: any) => {
      ...
      const changed =
        JSON.stringify([...excludedUsers].sort()) !== JSON.stringify([...prevUsers].sort()) ||
    ...
    if (db && dbSchema) {
      for (const r of results) {
        await db
          .insert(dbSchema.caExclusionSnapshots)
          .values({
            clientId: req.params.id,
            userId: req.userId ?? 'dev',
            policyId: r.policyId,
            policyName: r.policyName,
            excludedUsers: r.excludedUsers,
          ...
          .onConflictDoUpdate({
            target: [
              dbSchema.caExclusionSnapshots.clientId,
              dbSchema.caExclusionSnapshots.policyId,
            ],
            set: {
              policyName: r.policyName,
              excludedUsers: r.excludedUsers,
    ...
    res.json({
      ...
      withChanges: results.filter((r: any) => r.changed).length,
    })
  } catch (err: any) {
...
app.put(
  '/api/clients/:id/ca-exclusions/:policyId/justify',
  async (req: AuthedRequest, res: Response) => {
    ...
    if (db && dbSchema) {
      const { eq, and } = drizzleOps!
      await db
        .update(dbSchema.caExclusionSnapshots)
        .set({ justification })
        .where(
          and(
            eq(dbSchema.caExclusionSnapshots.clientId, id),
            eq(dbSchema.caExclusionSnapshots.policyId, policyId)
          )
        )
    }
    res.json({ ok: true })
...
app.get('/api/clients/:id/access-reviews', async (req: AuthedRequest, res: Response) => {
  try {
    ...
    let definitions: any[] = []
    try {
      const raw = await gc.query<any>('/identityGovernance/accessReviews/definitions', {
        select: ['id', 'displayName', 'scope', 'schedule', 'status'],
        top: 50,
      })
      ...
      return void res.json({
        supported: false,
        message: 'Access reviews require Entra ID P2 or Governance licensing.',
        definitions: [],
      })
    }

    const enriched = await Promise.all(
      definitions.map(async (d: any) => {
        let lastInstance: any = null
        try {
          const inst = await gc.query<any>(
            `/identityGovernance/accessReviews/definitions/${d.id}/instances`,
            { select: ['id', 'startDateTime', 'endDateTime', 'status'], top: 1 }
          )
        ...
        const daysSinceLast = lastCompleted
          ...
          : null
        const overdue =
          intervalDays && daysSinceLast !== null ? daysSinceLast > intervalDays * 1.1 : false
...
app.post('/api/clients/:id/tickets/scan', async (req: AuthedRequest, res: Response) => {
  ...
  const {
    ...
    frameworkId = 'CMMC_L2',
    projectKey,
  } = req.body as { platform: string; frameworkId?: string; projectKey?: string }
  ...
  try {
    ...
    if (db && dbSchema) {
      ...
      const rows = await db
        .select()
        .from(dbSchema.clientIntegrations)
        .where(
          and(
            eq(dbSchema.clientIntegrations.clientId, id),
            eq(dbSchema.clientIntegrations.platform, platform)
          )
        )
      integConfig = rows[0]?.config ?? null
    }
    if (!integConfig)
      return void res.status(400).json({ error: `No ${platform} integration configured` })
    ...
    if (platform === 'jira') {
      ...
      const jiraRes = await fetch(
        `https://${domain}/rest/api/3/search?jql=${encodeURIComponent(jql)}&maxResults=50&fields=summary,description`,
        {
          headers: {
      ...
      if (!jiraRes.ok)
        return void res.status(400).json({ error: 'Jira API error: ' + (await jiraRes.text()) })
      ...
      const snRes = await fetch(
        `${instanceUrl}/api/now/table/incident?sysparm_limit=50&sysparm_fields=number,short_description,description,sys_id`,
        {
          headers: {
      ...
      if (!snRes.ok)
        return void res.status(400).json({ error: 'ServiceNow API error: ' + (await snRes.text()) })
    ...
    for (const ticket of tickets) {
      ...
      const scored = controls
        .map((ctrl) => ({
          ctrl,
          confidence: scoreTicketVsControl(ticketText, `${ctrl.title} ${ctrl.description}`),
        }))
    ...
    if (db && dbSchema) {
      ...
      for (const nom of nominations) {
        ...
        const existing = await db
          .select({ id: dbSchema.ticketNominations.id })
...
app.put(
  '/api/clients/:id/tickets/nominations/:nomId',
  async (req: AuthedRequest, res: Response) => {
    ...
    if (db && dbSchema) {
      const { eq, and } = drizzleOps!
      await db
        .update(dbSchema.ticketNominations)
        .set({ status })
        .where(
          and(eq(dbSchema.ticketNominations.id, nomId), eq(dbSchema.ticketNominations.clientId, id))
        )
    }
    res.json({ ok: true })
  }
...
async function start() {
  ...
  app.listen(PORT, () => {
    void (async () => {
      ...
      console.log(
        `[INDEX] Clients         →  ${clients.length} configured${clients.length ? ` (${clients.map((c) => c.name).join(', ')})` : ' — open http://localhost:3000/setup'}`
      )
      const anthropicKey = resolveAnthropicKey()
      console.log(
        `[INDEX] Anthropic API   →  ${anthropicKey ? '✓ key present' : '✗ key not found (set ANTHROPIC_API_KEY)'}`
      )
